...
public class CaffeineCacheManager {
    ...
    public Cache<String, Object> build(
        ...
        Caffeine<Object, Object> caffeine = Caffeine.newBuilder()
            // 设置缓存初始容量
            .initialCapacity(Optional.ofNullable(initialCapacity).filter(s -> s >= 0).orElseGet(caffeineProperties::getInitialCapacity));

        // 设置缓存最大限度
        switch (maximumPolicy) {
            case MAXiMUM_SIZE:
                // 设置缓存最大容量
                caffeine.maximumSize(Optional.ofNullable(maximumSize).filter(s -> s >= 0).orElseGet(caffeineProperties::getMaximumSize));
                break;
            case MAXIMUM_WEIGHT:
                // 设置缓存最大权重
                caffeine.maximumWeight(Optional.ofNullable(maximumWeight).filter(s -> s >= 0).orElseGet(caffeineProperties::getMaximumWeight))
                    // 设置缓存权重计算器
                    .weigher(weigher);
                break;
        }
