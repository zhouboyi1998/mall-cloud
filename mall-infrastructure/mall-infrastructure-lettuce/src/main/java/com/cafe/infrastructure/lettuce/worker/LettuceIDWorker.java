package com.cafe.infrastructure.lettuce.worker;

import com.cafe.common.constant.pool.StringConstant;
import com.cafe.common.constant.redis.LettuceConstant;
import com.cafe.infrastructure.lettuce.util.LettuceExecutor;
 ...
 * @Package: com.cafe.infrastructure.lettuce.worker
 * @Author: zhouboyi
 * @Date: 2025/9/23 15:42
 * @Description: Lettuce 分布式ID生成器
 */
public class LettuceIDWorker {
    ...
    public LettuceIDWorker(long workerId, long datacenterId, RedisClient redisClient) {
        // 工作机器ID超出限制
    ...
    public long nextId(String group) {
        ...
        // 组装完整的 Redis Key
        String key = LettuceConstant.ID_PREFIX + datestamp + StringConstant.COLON + group;
        // 序列号递增
        long sequence = Optional.ofNullable(LettuceExecutor.execute(redisClient, redisCommands -> redisCommands.incr(key)))
            .map(increment -> increment & SEQUENCE_MASK)
            .orElseThrow(() -> new RuntimeException("Lettuce failed to generate a distributed increment id."));
     ...
     */
    public long nextId() {
        return nextId(LettuceConstant.DEFAULT_ID_GROUP);
    }
    ...
    public Long removeExpiredKeys() {
        ...
        // 组装 Redis Key 匹配规则
        String pattern = LettuceConstant.ID_PREFIX + StringConstant.ASTERISK + StringConstant.COLON + StringConstant.ASTERISK;
        // 获取所有匹配的 Redis Key
        List<String> keyList = LettuceExecutor.execute(redisClient, redisCommands -> redisCommands.keys(pattern));

        // 从列表中过滤出所有 7 天之前的 Redis Key
        keyList.removeIf(key -> Long.parseLong(key.substring(LettuceConstant.ID_PREFIX.length()).split(StringConstant.COLON)[0]) + 7 > datestamp);
        // 批量删除 Redis Key
        return CollectionUtils.isEmpty(keyList) ? 0L : LettuceExecutor.execute(redisClient, redisCommands -> redisCommands.del(keyList.toArray(new String[0])));
    }
}
