...
public class IdempotenceAspect {
    ...
    public Object doAround(ProceedingJoinPoint proceedingJoinPoint) {
        ...
        Boolean absent = Optional.ofNullable(redisTemplate.opsForValue().setIfAbsent(key, StringConstant.EMPTY, idempotence.intervalTime(), idempotence.intervalUnit()))
            .orElse(Boolean.FALSE);
        if (absent) {
            // 如果幂等 Key 不存在, 放行本次请求
            return proceedingJoinPoint.proceed();
        } else {
            // 如果幂等 Key 已存在, 判断本次请求为重复提交
            log.warn("IdempotenceAspect.doAround(): Duplicate request! idempotence key -> {}", key);
