...
import com.cafe.gateway.util.AuthorizationUtil;
import com.cafe.gateway.util.JWTUtil;
...
public class AuthorizationGlobalFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        // 获取 Access Token
        String accessToken = AuthorizationUtil.getAccessToken(request);
        // 如果 Access Token 为空, 直接返回
        if (StringUtils.isEmpty(accessToken)) {
            return chain.filter(exchange);
        }
        // 获取 JWT 载荷
        String payload = JWTUtil.getPayload(accessToken);
        // 打印日志
